-- Author:        Mario Castro
-- Caption:       Modelo Base de Datos Nomina Profesores OSJ Carabobo
-- Project:       Base de Datos Nomina Profesores OSJ Carabobo
-- Changed:       2012-04-09 09:03
...
CREATE TABLE dias(
  ...
  profesor_idprofesor INTEGER PRIMARY KEY NOT NULL CHECK(profesor_idprofesor>=0),
...
CREATE TABLE asistencia(
  ...
  profesor_idprofesor INTEGER PRIMARY KEY NOT NULL CHECK(profesor_idprofesor>=0),
  catedraasistencia INTEGER NOT NULL,
  observaciones VARCHAR(200),
  FOREIGN KEY(profesor_idprofesor)
    REFERENCES profesor(idprofesor)
    ON DELETE CASCADE
    ON UPDATE CASCADE
  FOREIGN KEY(catedraasistencia)
